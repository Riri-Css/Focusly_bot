...
// 🆕 New: The missing daily reset and streak logic
async function handleDailyCheckinReset(user) {
    ...
    if (lastCheckinDate && lastCheckinDate.isBefore(today, 'day')) {
        ...
        if (lastChecklist && lastChecklist.checkedIn) {
            // User checked in yesterday, so increase streak
            user.checkinStreak += 1;
            user.missedCheckins = 0; // Reset missed checkins
            console.log(`✅ User ${user.telegramId} streak increased to ${user.checkinStreak}`);
        } else {
            // User did not check in yesterday, so reset streak and increase missed count
